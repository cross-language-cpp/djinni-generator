...
| `--cpp-json-serialization <nlohmann_json>` | If specified, generate serializers to/from JSON and C++ types using [nlohmann/json](https://github.com/nlohmann/json). Requires also header-only [hinnant/date](https://github.com/HowardHinnant/date) for date serialization/deserialization.|
