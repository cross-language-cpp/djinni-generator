...
class MyRecordHelper:
    ...
    def get_my_record_f5(cself):
        try:
            _ret = CPyEnum.fromPy(CPyRecord.toPy(None, cself).my_enum)
            assert _ret != -1
            return _ret
        except Exception as _djinni_py_e:
            CPyException.setExceptionFromPy(_djinni_py_e)
    ...
    @ffi.callback("struct DjinniRecordHandle *(int32_t,struct DjinniString *,struct DjinniObjectHandle *,struct DjinniObjectHandle *,int)")
    def create_my_record(id,info,store,hash,my_enum):
        py_rec = MyRecord(
