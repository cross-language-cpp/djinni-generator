...
class ListBoolHelper:
    c_data_set = MultiSet()

    @staticmethod
    def check_c_data_set_empty():
        assert len(ListBoolHelper.c_data_set) == 0

    @ffi.callback("bool(struct DjinniObjectHandle *, size_t)")
    ...
    def __delete(c_ptr):
        assert c_ptr in ListBoolHelper.c_data_set
        ListBoolHelper.c_data_set.remove(c_ptr)
