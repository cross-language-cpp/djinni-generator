...
class CWrapperGenerator(spec: Spec) extends Generator(spec) {
  ...
  def getCppDefArgs(m: Interface.Method, self: String) = {
    if (m.static || self == "") {
      marshal.cArgDecl(
        m.params.map(p =>
          cppMarshal.fqParamType(p.ty) + " " + idCpp.local(p.ident.name)
        )
      )
    } else {
      marshal.cArgDecl(
  ...
  def getDefArgs(m: Interface.Method, self: String, forHeader: Boolean) = {
    if (m.static || self == "") {
      marshal.cArgDecl(
        m.params.map(p =>
          marshal.cParamType(p.ty, forHeader) + " " + idCpp.local(p.ident.name)
        )
      )
    } else {
      marshal.cArgDecl(
        Seq[String](self) ++
          m.params.map(p =>
            marshal.cParamType(p.ty, forHeader) + " " + idCpp.local(
              p.ident.name
            )
          )
      )
  ...
  def declareGlobalGetter(
    ...
    w.wl(
      "static " + ret + " " + p(
        " * " + marshal.callback(idCpp.method(className + methodName))
      ) + cArgs + ";"
    )
    w.wl
    w.wl(
      "void " + idCpp.method(className + "_add_callback" + methodName) + p(
        methodSignature
      ) + " {"
    ).nested {
      w.wl(marshal.callback(idCpp.method(className + methodName)) + " = ptr;")
    }
    w.wl("}")
    w.wl
  }

  def declareGlobalGetterSignature(
      methodName: String,
      ret: String,
      cArgs: String,
      className: String,
  ...
  def writeWrapHandleAsUniquePointer(
      tm: MExpr,
      createMethod: String,
      w: IndentWriter
  ): Unit = {
    w.wl(
      "djinni::Handle" + t("DjinniObjectHandle") + " _handle(" +
        createMethod + ", " + "& " + marshal.getReleaseMethodName(tm) + ");"
    )
  }

  def declareObjectUniquePointerHandle(
      tm: MExpr,
  ...
  def getUniquePointerDeclarationIfNeeded(
      elName: String,
      elToFrom: String,
      elTy: MExpr
  ): String = {
    ...
    val decl = if (marshal.needsRAII(elTy)) {
      if (marshal.canRAIIUseStandardUniquePtr(elTy)) {
        ...
        "std::unique_ptr" + t(
          marshal.removePointer(marshal.paramType(elTy))
        ) + indentedElName + p(elToFrom)
      } else {
        "djinni::Handle" + t(
          marshal.removePointer(marshal.paramType(elTy))
        ) + indentedElName + p(
  ...
  def declareUniquePointer(
    ...
    val declaration =
      getUniquePointerDeclarationIfNeeded(elName, elToFrom, elTy)
  ...
  def writeListToCpp(
    ...
    w.wl(retType + "_ret;")
    w.wl(
      "size_t size = " + marshal.callback(
        idCpp.method(classAsMethodName) + idCpp.method("__get_size") + p(
          "dh.get()"
        ) + ";"
      )
    )
      ...
      w.wl(
        "_ret.push_back" + p(
          marshal.convertTo(
            raiiForElement(
              marshal.callback(
                idCpp.method(classAsMethodName) + idCpp
                  .method("__get_elem") + p("dh.get(), i")
              ),
              tm.args(0)
            ),
            elTyRef,
  ...
  def writeListFromCpp(
    ...
    w.wl("for (int i = 0; i < size; i++) {").nested {
      if (marshal.needsRAII(elTyRef)) {
        ...
        w.wl(
          addToList + p(
            "_handle.get(), " + marshal.convertFrom("dc[i]", elTyRef)
          ) + ";"
        )
      }
    }
  ...
  def writeMapToCpp(
    ...
    w.wl(
      "size_t size = " + marshal.callback(
        idCpp.method(className) + idCpp.method("__get_size") + p(
          "dh.get()"
        ) + ";"
      )
    )
    ...
    w.wl("for (int i = 0; i < size; i++) {").nested {
      w.wl(
        "auto _key_c = " + raiiForElement(
          marshal.callback(
            idCpp.method(className) + idCpp.method("__next") + p("dh.get()")
          ),
          tm.args(0)
        ) + "; // key that would potentially be surrounded by unique pointer"
      )
      val key =
        if (marshal.needsRAII(tm.args(0))) "_key_c.get()"
        else "_key_c" // don't delete ownership
      w.wl(
        "auto _val = " + marshal.convertTo(
          raiiForElement(
            marshal.callback(
              idCpp.method(className) + idCpp.method("__get_value") + p(
                "dh.get(), " + key
              )
            ),
            tm.args(1)
  ...
  def declareGlobalGetters(
    ...
    declareGlobalGetter("__create", ret, cArgs, className, w)

    ret = "void"
    cArgs = marshal.cArgDecl(Seq(handle, addCArgs))
    declareGlobalGetter("__add", ret, cArgs, className, w)
  }

  def declareGlobalGettersSignatures(
      tm: MExpr,
      ident: Ident,
      retType: String,
      className: String,
    ...
    cArgs = marshal.cArgDecl(Seq())
    declareGlobalGetterSignature("__create", ret, cArgs, className, w)

    ret = "void"
    cArgs = marshal.cArgDecl(Seq(handle, addCArgs))
    declareGlobalGetterSignature("__add", ret, cArgs, className, w)
  }

  def writeObjectReleaseMethodSignature(
      objectAsMethodName: String,
      objectHandle: String,
      w: IndentWriter
  ): Unit = {
    w.wl("void " + objectAsMethodName + "___delete" + p(objectHandle) + ";")
    w.wl(
      "void " + "optional_" + objectAsMethodName + "___delete" + p(
        "struct DjinniOptionalObjectHandle *"
      ) + ";"
    )
    w.wl(
      "void " + objectAsMethodName + "_add_callback_" + "__delete" + p(
        "void(* ptr)" + p("struct " + marshal.djinniObjectHandle + " * ")
      ) + ";"
    )
  }

  def writeObjectReleaseMethods(
      objectAsMethodName: String,
      objectHandlePtr: String,
      w: IndentWriter
  ): Unit = {
    // Callback for delete front-end object handle
    w.wl(
      "static void(*" + marshal.callback(
        objectAsMethodName + "___delete"
      ) + ")" + p(objectHandlePtr) + ";"
    )
    w.wl(
      "void " + objectAsMethodName + "_add_callback_" + "__delete" + p(
        "void(* ptr)" + p(objectHandlePtr)
      ) + " {"
    ).nested {
      w.wl(marshal.callback(objectAsMethodName + "___delete") + " = ptr;")
    }
    w.wl("}")
    w.wl
    ...
    w.wl(
      "void " + "optional_" + objectAsMethodName + "___delete" + cArgs + " {"
    ).nested {
      w.wl(
        marshal.callback(objectAsMethodName + "___delete") + p(
          p(objectHandlePtr) + " drh"
        ) + ";"
      )
    }
    w.wl("}")
  ...
  def writeOptionalContainerFromCpp(
      optHandle: String,
      handle: String,
      retType: String,
    ...
    w.wl(
      ...
      w.wl("}")
      w.wl(
        "return " + "djinni::optionals::toOptionalHandle" + p(
          djinniWrapper + "::fromCpp" + p(
            "std::move(* dc)"
          ) + ", " + deleteMethod
        ) + ";"
      )
  ...
  def writeOptionalContainerToCpp(
      optHandle: String,
      retTypeStr: String,
      djinniWrapper: String,
      reinterpret: String,
    ...
    w.wl(
      ...
      w.wl(" if (dh) {").nested {
        w.wl(
          "return " + spec.cppOptionalTemplate + t(retTypeStr) + p(
            djinniWrapper + "::toCpp" + p(
              "djinni::optionals::fromOptionalHandle(std::move(dh), " + deleteMethod + ")"
            )
          ) + ";"
        )
      }
  ...
  def writeContainerFromToCpp(
    ...
    w.wl(
      "static " + retType + " toCpp" + p(
        "djinni::Handle" + t(cppHandle) + " dh"
      ) + ";"
    )

    w.wl(
      "static " + "djinni::Handle" + t(cppOptHandle) + "fromCpp" + p(
        spec.cppOptionalTemplate + t(retType) + " dc"
      ) + ";"
    )
    w.wl(
      "static " + spec.cppOptionalTemplate + t(retType) + " toCpp" + p(
        "djinni::Handle" + t(cppOptHandle) + " dh"
      ) + ";"
    )
  }
  ...
  def generateList(
    ...
    var listElType =
      marshal.cParamType(getContainerElTypeRef(tm, 0, ident), true)
    ...
    writeCHeader(
      ...
      w => {
        ...
        declareGlobalGettersSignatures(
          tm,
          ident,
          retType,
          classAsMethodName,
    ...
    writeCFile(
      ...
      w => {
        ...
        // List from cpp
        w.wl(
          "djinni::Handle" + t(handle) + " " + djinniWrapper + "::fromCpp" + p(
            cppMarshal.fqParamType(tm) + " dc"
          ) + " {"
        ).nested {
        ...
        // List to cpp
        w.wl(
          retType + " " + djinniWrapper + "::toCpp" + p(
            "djinni::Handle" + t(handle) + " dh"
          ) + " {"
        ).nested {
        ...
        w.wl("}")
        w.wl
        writeOptionalContainerFromCpp(
          optHandle,
          handle,
          retType,
          djinniWrapper,
          "optional_" + marshal.getReleaseMethodName(tm),
          w
        )
        writeOptionalContainerToCpp(
          optHandle,
          retType,
          djinniWrapper,
          handlePtr,
          marshal.getReleaseMethodName(tm),
          w
        )
      }
    )
  ...
  def generateMap(
    ...
    writeCHeader(
      ...
      w => {
        ...
        declareGlobalGetterSignature("__next", ret, cArgs, classAsMethodName, w)
      }
    )
    ...
    writeCFile(
      ...
      w => {
        ...
        declareGlobalGetter("__next", ret, cArgs, classAsMethodName, w)
  ...
  class CRefs(ident: Ident, origin: String) {
    ...
    def collect(tm: MExpr, justCollect: Boolean) {
      ...
      if (justCollect) {
        ...
        if (
          !Generator.writtenFiles.contains(
            (marshal.dh + name + ".hpp").toLowerCase()
          )
        ) {
          Generator.writtenFiles.put(fileName.toLowerCase(), fileName)
          tm.base match {
            case MList =>
              generateList(tm, name, className, ident, origin, h, hpp)
  ...
  def declareGlobalFunctionPointers(
      ident: Ident,
      cClassWrapper: String,
    ...
    for (m <- methods) {
      ...
      w.wl(
        "static " + ret + "(*" + marshal.callback(
          idCpp.method(ident.name) + "_" + idCpp.method(m.ident.name)
        ) + ")" + cArgs + ";"
      )
    }
    ...
    w.wl(
      "static void(*" + marshal.callback(
        idCpp.method(ident.name) + "___delete"
      ) + ")" + p(marshal.djinniObjectHandle + " * ") + ";"
    )
  }

  def declareGlobalRecordFieldGetters(
      ident: Ident,
      r: Record,
      prefix: String,
      number: Int,
  ...
  def declareGlobalSignaturesRecordFieldGetters(
      ident: Ident,
      r: Record,
      prefix: String,
      number: Int,
  ...
  def writeRecordToCpp(
    ...
    for (f <- r.fields) {
      ...
      val f_id = prefix + field_number.toString()
      val methodName =
        recordAsMethodName + "_" + "get_" + ident.name + "_f" + f_id
      if (marshal.needsRAII(f.ty)) {
        declareUniquePointer(
          "_field_" + idCpp.method(f.ident.name),
          marshal.callback(idCpp.method(methodName) + p("dh.get()")),
          f.ty.resolved,
          w
        )
      }
    ...
    w.wl("auto _aux = " + recordAsFqClassType + "(").nested {
      ...
      for (f <- r.fields) {
        ...
        if (marshal.needsRAII(f.ty)) {
          w.w(
            marshal.convertTo(" _field_" + idCpp.method(f.ident.name), f.ty)
          ) // RAII done above
        } else {
          w.w(
            marshal.convertTo(
  ...
  def writeRecordFromCpp(
    ...
    for (f <- r.fields) {
      if (marshal.needsRAII(f.ty)) {
        w.wl(
          "auto  _field_" + idCpp.method(f.ident.name) + " = " + marshal
            .convertFrom("dr." + f.ident.name, f.ty) + ";"
        )
      }
    }
    w.wl
    w.wl("djinni::Handle" + t("DjinniRecordHandle") + " _aux(").nested {
      w.wl(
        marshal.callback(
          recordAsMethodName + idCpp.method("_create_") + recordAsMethodName
        ) + "("
      ).nested {
        val skipFirst = SkipFirst()
        for (f <- r.fields) {
          ...
          if (marshal.needsRAII(f.ty)) {
            ...
            w.w(marshal.convertFrom("dr." + idCpp.local(f.ident.name), f.ty))
    ...
    if (create)
      createFile(
        ...
        (w: IndentWriter) => {
          ...
          w.wl(
            "#include " + q(
              cWrapperBaseLibIncludePrefix + "wrapper_marshal.hpp"
            )
          )
          w.wl(
            "#include " + q(spec.cWrapperIncludeCppPrefix + ident + ".hpp")
          ) // include abstract cpp base class header
    ...
    if (create)
      createFile(
        ...
        (w: IndentWriter) => {
          ...
          w.wl(
            "#include <stdbool.h> " + marshal.pythonCdefIgnore
          ) // seeing if 2 handles are equal needs this
          if (includes.nonEmpty) {
            w.wl
      ...
      createFile(
        ...
        (w: IndentWriter) => {
          ...
          writeAsExternC(
            ...
            w => {
              w.wl(
                "#include " + q(spec.cWrapperIncludePrefix + fileName + ".h")
              ) // include own header
            }
          )
        }
  ...
  def writeMethodSignatures(
      methods: Seq[Interface.Method],
      forHeader: Boolean,
      forCpp: Boolean,
      cWrapper: String,
      cMethodWrapper: String,
      w: IndentWriter
  ): Unit = {
    ...
    for (m <- methods) {
      val ret =
        if (forCpp) cppMarshal.fqReturnType(m.ret)
        else marshal.cReturnType(m.ret, forHeader)

      val params =
        if (forCpp) getCppDefArgs(m, self) else getDefArgs(m, self, true)
      w.wl(
        ret + " " + cMethodWrapper + idCpp.method(m.ident.name) + params + ";"
      )
      w.wl
    }
  }

  def writeGetWrappedObj(
      ident: Ident,
      w: IndentWriter,
      className: String,
      ext: Ext
    ...
    w.wl(
      "std::shared_ptr<" + withCppNs(className) + "> " + marshal.wrappedName(
        className
      ) + "::get" + p(
        "djinni::WrapperRef" + t(marshal.wrappedName(className)) + " dw"
      ) + " {"
    ).nested {
      w.wl("if (dw) {").nested {
    ...
    if (ext.py) {
      w.wl(
        "void " + idCpp.method(ident.name) + "___delete" + p(
          marshal.djinniObjectHandle + " * dh"
        ) + " {"
      ).nested {
        // Only if we implement the interface in front-end, will there be a delete from front-end callback
        w.wl(
          marshal.callback(idCpp.method(ident.name) + "___delete") + p(
            "dh"
          ) + ";"
        )
      }
    ...
    w.wl(
      "void " + idCpp.method(ident.name) + "___wrapper_add_ref" + p(
        marshal.wrappedName(className) + " * dh"
      ) + " {"
    ).nested {
    ...
    w.wl(
      "void " + idCpp.method(ident.name) + "___wrapper_dec_ref" + p(
        marshal.wrappedName(className) + " * dh"
      ) + " {"
    ).nested {
    ...
    w.wl(
      "djinni::Handle" + t(marshal.wrappedName(className)) + " " + marshal
        .wrappedName(className) + "::wrap" + p(
        "std::shared_ptr<" + withCppNs(className) + "> obj"
      ) + " {"
    ).nested {
      w.wl("if (obj)").nested {
        w.wl(
          "return " + "djinni::Handle" + t(marshal.wrappedName(className)) + p(
            "new " + marshal.wrappedName(className) + "{ std::move(obj) }, " +
    ...
    w.wl("struct " + marshal.wrappedName(className) + " final {").nested {
      w.wl(
        marshal.wrappedName(className) + p(
          "std::shared_ptr" + t(withCppNs(className)) + "wo"
        ) + ": wrapped_obj" + p("wo") + " {};"
      )
      w.wl
      w.wl(
        "static std::shared_ptr" + t(withCppNs(className)) + " " + "get" + p(
          "djinni::Handle" + t(marshal.wrappedName(className)) + " dw"
        ) + ";"
      )
      w.wl(
        "static djinni::Handle" + t(
          marshal.wrappedName(className)
        ) + " wrap" + p(
          "std::shared_ptr" + t(withCppNs(className)) + " obj"
        ) + ";"
      )
      w.wl
      w.wl(
  ...
  def writeDjinniUnwrapperSignature(
      cMethodWrapper: String,
      cWrapper: String,
      ext: Ext,
      w: IndentWriter
    ...
    if (ext.py) {
      w.wl(
        "void " + cMethodWrapper + "___delete(" + "struct " + marshal.djinniObjectHandle + " * " + dw + ");"
      )
    }
    w.wl(
      "void " + idCpp.method(cMethodWrapper) + "___wrapper_dec_ref" + p(
        "struct " + cWrapper + " * dh"
      ) + ";"
    )
    w.wl(
      "void " + idCpp.method(cMethodWrapper) + "___wrapper_add_ref" + p(
        "struct " + cWrapper + " * dh"
      ) + ";"
    )
  }
  ...
  def checkNonOptionals(m: Interface.Method, w: IndentWriter): Unit = {
    m.params.map(arg =>
      if (arg.ty.resolved.base != MOptional)
        w.wl("assert " + p(arg.ident.name + " != nullptr") + ";")
    )
  }

  def writeMethodToCpp(
      m: Interface.Method,
      fctPrefix: String,
      cWrapper: String,
      cMethodWrapper: String,
    ...
    val args = m.params.map(p =>
      marshal.convertTo(
        (if (marshal.needsRAII(p.ty.resolved)) "_" else "") + p.ident.name,
        p.ty,
        tempExpr = false
      )
    ) // x
    val fctCall =
      fctPrefix + idCpp.method(m.ident.name) + marshal.cArgVals(args)
    val returnStmt =
      if (m.ret.isEmpty) fctCall + ";"
      else
        "return " + marshal.convertFrom(
          fctCall,
          m.ret.get,
          tempExpr = true
        ) + (if (marshal.needsRAII(m.ret.get)) ".release()" else "") + ";"

    val params = getDefArgs(m, cWrapper + " * " + dw, false)
    w.wl(
      ret + " " + cMethodWrapper + "_" + idCpp.method(
        m.ident.name
      ) + params + " {"
    ).nested {
      // take ownership of arguments memory when arguments come from front-end
      m.params.foreach(p =>
        declareUniquePointer("_" + p.ident.name, p.ident.name, p.ty.resolved, w)
      )
      w.wl("try {").nested {
        w.wl(returnStmt)
      }
  ...
  def writeMethodFromCpp(
    ...
    val args = marshal.cArgDecl(
      m.params.map(p =>
        cppMarshal.fqParamType(p.ty) + " " + idCpp.local(p.ident.name)
      )
    )
    val params = Seq(handle) ++ m.params.map(p =>
      if (marshal.needsRAII(p.ty.resolved))
        "_" + idCpp.local(p.ident.name) + ".release()"
      else marshal.convertFrom(idCpp.local(p.ident.name), p.ty)
    )

    w.wl(ret + " " + cppClass + "::" + idCpp.method(m.ident.name) + args + " {")
      .nested {
        // take ownership of arguments memory when arguments come from front-end
        m.params.foreach(p =>
          if (marshal.needsRAII(p.ty))
            w.wl(
              "auto _" + p.ident.name + " = " + marshal
                .convertFrom(p.ident.name, p.ty) + ";"
            )
        )

        val method_call = marshal.callback(
          idCpp.method(ident.name) + "_" + idCpp.method(m.ident.name)
        ) + marshal.cArgVals(params)
        if (m.ret.isDefined) {
          val method_call = marshal.callback(
            idCpp.method(ident.name) + "_" + idCpp.method(m.ident.name)
          ) + marshal.cArgVals(params)
          w.wl(
            "auto _ret = " + marshal.convertTo(
              raiiForElement(method_call, m.ret.get.resolved),
              m.ret.get,
              tempExpr = true
            ) + ";"
          )
  ...
  def writeGetSetEqHandle(
      djinniWrapper: String,
      cMethodName: String,
      cFrontEndProxy: String,
      w: IndentWriter
  ): Unit = {

    // Function to return handle around FrontEndProxy holding a front-end object
    w.wl(
      djinniWrapper + " * " + " make_proxy_object_from_handle_" + cMethodName + p(
        marshal.djinniObjectHandle + " * " + "c_ptr"
      ) + " {"
    ).nested {
      w.wl(
        "return new " + djinniWrapper + "{" + "std::make_shared<" + cFrontEndProxy + ">(c_ptr)};"
      )
    }
    w.wl("}")
    w.wl
    ...
    w.wl(
      marshal.djinniObjectHandle + " * " + "get_handle_from_proxy_object_" + cMethodName + p(
        djinniWrapper + " * dw"
      ) + " {"
    ).nested {
      w.wl(
        cFrontEndProxy + " * " + "cast_ptr = dynamic_cast" + t(
          cFrontEndProxy + " * "
        ) + p("dw->wrapped_obj.get()") + ";"
      )
    ...
    w.wl
    w.wl(
      "bool " + "equal_handles_" + cMethodName + p(
        djinniWrapper + " * dw1, " +
          djinniWrapper + " * dw2"
      ) + " {"
    ).nested {
  ...
  def writeGetSetEqHandleSignatures(
      djinniWrapper: String,
      cMethodName: String,
      w: IndentWriter
  ) = {
    ...
    w.wl(
      "struct " + marshal.djinniObjectHandle + " * " + "get_handle_from_proxy_object_" + cMethodName + p(
        "struct " + djinniWrapper + " * dw"
      ) + ";"
    )
    w.wl(
      "bool " + "equal_handles_" + cMethodName + p(
        "struct " + djinniWrapper + " * dw1, " +
  ...
  def writeUnimplementableInterface(
      ident: Ident,
      origin: String,
      cppClass: String,
      refs: CRefs,
    ...
    refs.hpp.add(
      "#include " + q(
        marshal.cw + ident.name + ".hpp"
      ) + marshal.pythonCdefIgnore
    ) // make sure own header included (for static)

    // C header for cdef
    writeCHeader(
      marshal.cw + ident.name,
      origin,
      cppClass,
      refs.h,
      true,
      w => {
        ...
        writeDjinniUnwrapperSignature(
          idCpp.method(ident.name),
          cClassWrapper,
          i.ext,
          w
        )
  ...
  def writeCFromCpp(
    ...
    refs.hpp.add(
      "#include " + q(marshal.cw + ident.name + ".hpp")
    ) // make sure own header included
      ...
      w => {
        ...
        for (m <- i.methods) {
          ...
          val cArgs = getCArgTypes(
            m,
            "struct " + marshal.djinniObjectHandle + " * ",
            true
          )
          val methodSignature = ret + "(* ptr)" + cArgs
          w.wl(
            "void " + classAsMethodName + "_add_callback_" + idCpp.method(
              m.ident.name
            ) + p(methodSignature) + ";"
          )
          w.wl
        ...
        w.wl(
          "void " + classAsMethodName + "_add_callback_" + "__delete" + p(
            "void(* ptr)" + p("struct " + marshal.djinniObjectHandle + " * ")
          ) + ";"
        )
      }
    ...
    writeCppHeader(
      ...
      w => {
        writeDjinniWrapper(
          ...
          w,
          cppClass
        ) // not per se needed but helpful for allowing general dynamic casts
        w.wl
        // FrontEndProxy Implementation
        w.wl(
          "class " + cFrontEndProxy + " final : public " + withCppNs(
            className
          ) + " {"
        ).nested {
          w.wl("public:").nested {
    ...
    writeCFile(
      ...
      w => {
        ...
        w.wl(
          marshal.djinniObjectHandle + " * " + cFrontEndProxy + "::get_m_obj_handle() {"
        ).nested {
        ...
        w.wl(
          cFrontEndProxy + "::" + cFrontEndProxy + p(
            marshal.djinniObjectHandle + " * " + "c_ptr"
          ) + " : " + handle + p("c_ptr") + " {}"
        )
        w.wl
        ...
        w.wl(
          "void " + classAsMethodName + "_add_callback_" + "__delete" + p(
            "void(* ptr)" + p(marshal.djinniObjectHandle + " * ")
          ) + " {"
        ).nested {
          w.wl(
            marshal.callback(idCpp.method(ident.name) + "___delete") + " = ptr;"
          )
        }
        w.wl("}")
        w.wl
  ...
  def generateEnum(origin: String, ident: Ident, doc: Doc, e: Enum): Unit = {
    ...
    writeCppHeader(
      ...
      w => {
        w.wl(
          "int32_t int32_from_enum_" + idCpp.method(ident.name) + p(
            spec.cppOptionalTemplate + t(
              withCppNs(idCpp.enumType(ident.name))
            ) + " e"
          ) + ";"
        )
        w.wl(
          "int32_t int32_from_enum_" + idCpp.method(ident.name) + p(
            withCppNs(idCpp.enumType(ident.name)) + " e"
          ) + ";"
        )
        w.wl(
          spec.cppOptionalTemplate + t(withCppNs(idCpp.enumType(ident.name))) +
            " get_boxed_enum_" + idCpp.method(ident.name) + "_from_int32" + p(
              "int32_t e"
            ) + ";"
        )
      }
    ...
    writeCFile(
      ...
      w => {
        ...
        w.wl(
          ...
          w.wl("}")
          w.wl(
            "return " + spec.cppOptionalTemplate + t(
              withCppNs(idCpp.enumType(ident.name))
            ) +
              p(
                "static_cast" + t(withCppNs(idCpp.enumType(ident.name))) + p(
                  "e"
                )
              ) + ";"
          )
  ...
  def generateRecord(
    ...
    writeCHeader(
      ...
      w => {
        ...
        w.wl(
          "void " + recordAsMethodName + "___delete" + p(
            "struct " + recordHandle + " * "
          ) + ";"
        )
        w.wl(
          "void " + "optional_" + recordAsMethodName + "___delete" + p(
            "struct " + optRecordHandle + " * "
          ) + ";"
        )
    ...
    writeCppHeader(
      ...
      w => {
        w.wl("struct " + recordName + " {").nested {
          // from cpp
          w.wl(
            "static " + "djinni::Handle" + t(recordHandle) + " fromCpp" + p(
              "const " + fqCppRecordName + "&" + " dr"
            ) + ";"
          )
          // to cpp
          w.wl(
            "static " + fqCppRecordName + " " + "toCpp" + p(
              "djinni::Handle" + t(recordHandle) + " dh"
            ) + ";"
          )

          w.wl(
            "static " + "djinni::Handle" + t(optRecordHandle) + " fromCpp" + p(
              spec.cppOptionalTemplate + t(fqCppRecordName) + " dc"
            ) + ";"
          )
          w.wl(
            "static " + spec.cppOptionalTemplate + t(
              fqCppRecordName
            ) + " toCpp" + p(
              "djinni::Handle" + t(optRecordHandle) + " dh"
            ) + ";"
          )
    ...
    writeCFile(
      ...
      w => {
        ...
        w.wl(
          "static void(*" + marshal.callback(
            recordAsMethodName + "___delete"
          ) + ")" + p(recordHandle + " * ") + ";"
        )

        w.wl(
          "void " + recordAsMethodName + "_add_callback_" + "__delete" + p(
            "void(* ptr)" + p(recordHandle + " * ")
          ) + " {"
        ).nested {
          w.wl(
            marshal.callback(idCpp.method(ident.name) + "___delete") + " = ptr;"
        ...
        w.wl("void " + idCpp.method(ident.name + "___delete") + cArgs + " {")
          .nested {
            w.wl(
              marshal.callback(idCpp.method(ident.name) + "___delete") + p(
                "drh"
        ...
        w.wl(
          "void " + idCpp.method(
            "optional_" + ident.name + "___delete"
          ) + cArgs + " {"
        ).nested {
          w.wl(
            marshal.callback(idCpp.method(ident.name) + "___delete") + p(
              "(DjinniRecordHandle *) drh"
            ) + "; // can't static cast, find better way"
          )
        }
        w.wl("}")
        ...
        // to cpp
        w.wl(
          fqCppRecordName + " " + recordName + "::toCpp" + p(
            "djinni::Handle" + t(recordHandle) + " dh"
          ) + " {"
        ).nested {
        ...
        writeOptionalContainerFromCpp(
          ...
          fqCppRecordName,
          recordName,
          "optional_" + recordAsMethodName + "___delete",
          w
        )
        writeOptionalContainerToCpp(
          optRecordHandle,
