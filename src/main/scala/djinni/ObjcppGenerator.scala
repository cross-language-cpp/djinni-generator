...
class ObjcppGenerator(spec: Spec) extends BaseObjcGenerator(spec) {
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface) {
    ...
    writeObjcFile(ident.name, isHeader = true, origin, refs.privHeader, w => {
      arcAssert(w)
      w.wl
  ...
  override def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: Record) {
    ...
    writeObjcFile(objcName, isHeader = true, origin, refs.privHeader, w => {
      arcAssert(w)
      w.wl
    ...
    writeObjcFile(objcName, isHeader = false, origin, refs.body, w => {
      wrapNamespace(w, spec.objcppNamespace, w => {
  ...
  def writeObjcFile(objcName: String, isHeader: Boolean, origin: String, refs: Iterable[String], f: IndentWriter => Unit) {
    val folder = if (isHeader) spec.objcppHeaderOutFolder else spec.objcppOutFolder
    val fileName = if(isHeader) objcppMarshal.privateHeaderName(objcName) else privateBodyName(objcName)
