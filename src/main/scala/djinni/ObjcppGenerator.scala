...
class ObjcppGenerator(spec: Spec) extends BaseObjcGenerator(spec) {
  ...
  override def generateEnum(origin: String, ident: Ident, doc: Doc, e: Enum) {
    ...
    writeObjcFile(objcppMarshal.privateHeaderName(ident.name), isHeader = true, origin, imports, w => {} )
  }
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface) {
    ...
    writeObjcFile(objcppMarshal.privateHeaderName(ident.name), isHeader = true, origin, refs.privHeader, w => {
      arcAssert(w)
      w.wl
    ...
    writeObjcFile(privateBodyName(ident.name), isHeader = false, origin, refs.body, w => {
      arcAssert(w)
  ...
  override def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: Record) {
    ...
    writeObjcFile(objcppMarshal.privateHeaderName(objcName), isHeader = true, origin, refs.privHeader, w => {
      arcAssert(w)
      w.wl
    ...
    writeObjcFile(privateBodyName(objcName), isHeader = false, origin, refs.body, w => {
      wrapNamespace(w, spec.objcppNamespace, w => {
  ...
  def writeObjcFile(fileName: String, isHeader: Boolean, origin: String, refs: Iterable[String], f: IndentWriter => Unit) {
    val folder = if (isHeader) spec.objcppHeaderOutFolder else spec.objcppOutFolder
