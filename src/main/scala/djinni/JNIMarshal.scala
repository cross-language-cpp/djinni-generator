...
class JNIMarshal(spec: Spec) extends Marshal(spec) {
  ...
  def helperClass(name: String): String = spec.jniClassIdentStyle(name)
  ...
  def toJniType(m: MExpr, needRef: Boolean): String = m.base match {
    ...
    case _: MExtern =>
      helperClass(m) + (if (needRef) "::Boxed" else "") + "::JniType"
    case _ => "jobject"
  }
      ...
      o match {
        ...
        case MOptional =>
          tm.args.head.base match {
            ...
            case _             => javaTypeSignature(tm.args.head)
          }
  ...
  def isJavaHeapObject(ty: TypeRef): Boolean = isJavaHeapObject(
    ty.resolved.base
  )
