...
class ObjcMarshal(spec: Spec) extends Marshal(spec) {
  ...
  override def returnType(ret: Option[TypeRef]): String =
    ret.fold("void")((t: TypeRef) =>
      nullability(t.resolved).fold("")(_ + " ") + toObjcParamType(t.resolved)
    )
  ...
  override def toCpp(tm: MExpr, expr: String): String =
    throw new AssertionError("direct objc to cpp conversion not possible")
      ...
      case d: MDef =>
        d.defType match {
          ...
          case DInterface =>
            ...
            if (!ext.objc) {
              List(
                ImportRef("<Foundation/Foundation.h>"),
                DeclRef(s"@class ${typename(d.name, d.body)};", None)
              )
            } else {
              List(
                ImportRef("<Foundation/Foundation.h>"),
                DeclRef(s"@protocol ${typename(d.name, d.body)};", None)
              )
            }
          case DRecord =>
  ...
  def toObjcType(ty: TypeRef): (String, Boolean) =
    toObjcType(ty.resolved, false)
  def toObjcType(ty: TypeRef, needRef: Boolean): (String, Boolean) =
    toObjcType(ty.resolved, needRef)
  ...
  def toObjcType(tm: MExpr, needRef: Boolean): (String, Boolean) = {
    ...
    else tm.args.map(toBoxedParamType).mkString("<", ", ", ">")
    def f(tm: MExpr, needRef: Boolean): (String, Boolean) = {
      tm.base match {
        ...
        case o =>
          val base = o match {
            case p: MPrimitive =>
              if (needRef) (p.objcBoxed, true) else (p.objcName, false)
            ...
            case d: MDef =>
              d.defType match {
                case DEnum =>
                  if (needRef) ("NSNumber", true)
                  else (idObjc.ty(d.name), false)
            ...
            case e: MExtern =>
              e.body match {
                case i: Interface =>
                  if (i.ext.objc) (s"id<${e.objc.typename.get}>", false)
                  else (e.objc.typename.get, true)
                case _ =>
                  if (e.objc.generic.get)
  ...
  /** This method returns whether we can use global variable to represent a
    * given constant.
    *
    * We can use global variables for constants which are safe to create during
    * static init, which are numbers strings, and optional strings. Anything
    * else needs to be a class method.
    */
