...
class ObjcGenerator(spec: Spec) extends BaseObjcGenerator(spec) {
  ...
  override def generateEnum(origin: String, ident: Ident, doc: Doc, e: Enum) {
    ...
    writeObjcFile(ident, isHeader = true, origin, refs.header, w => {
      writeDoc(w, doc)
      w.wl(if(e.flags) {
  ...
  override def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: Record) {
    ...
    writeObjcFile(objcName, isHeader = true, origin, refs.header, w => {
      writeDoc(w, doc)
  ...
  def writeObjcFile(objcName: String, isHeader: Boolean, origin: String, refs: Iterable[String], f: IndentWriter => Unit) {
    ...
    val fileName = if (isHeader) marshal.headerName(objcName) else bodyName(objcName)
