...
class ObjcppMarshal(spec: Spec) extends Marshal(spec) {
  ...
  override def fqParamType(tm: MExpr): String = throw new AssertionError(
    "not applicable"
  )
  ...
  override def fieldType(tm: MExpr): String = throw new AssertionError(
    "not applicable"
  )
  override def fqFieldType(tm: MExpr): String = throw new AssertionError(
    "not applicable"
  )
  ...
  def privateHeaderName(ident: String): String =
    idObjc.ty(ident) + "+Private." + spec.objcHeaderExt
    ...
    case d: MDef =>
      d.defType match {
        case DEnum =>
          withNs(
            Some("djinni"),
            s"Enum<${cppMarshal.fqTypename(tm)}, ${objcMarshal.fqTypename(tm)}>"
          )
