...
class PythonMarshal(spec: Spec) extends Marshal(spec) {
  ...
  override def returnType(ret: Option[TypeRef]): String =
    ret.fold("void")(ty => toPythonType(ty.resolved))
...
//  override def fromCpp(tm: MExpr, expr: String): String = throw new AssertionError("direct cpp to python conversion not possible")
  ...
  def references(m: Meta, exclude: String): Seq[SymbolReference] = m match {
      ...
      if (idPython.local(d.name) != idPython.local(exclude)) {
        d.defType match {
          case DInterface =>
            List(
              ImportRef(
                "from " + spec.pyImportPrefix + idPython.local(
                  d.name
                ) + " import " + className
              ),
              ImportRef(
          ...
          case DRecord =>
            List(
              ...
              ImportRef(
                "from " + spec.pyImportPrefix + idPython.local(
                  d.name
                ) + "_helper" + " import " + className + "Helper"
              )
            )
          case DEnum =>
            List(
    ...
    case MBinary =>
      List(ImportRef("from djinni.pycffi_marshal import CPyBinary"))
      ...
      tm.base match {
        ...
        case MList => {
          ...
          if (idPython.className(idlName) != idPython.className(exclude)) {
            refs.add(
              "from " + spec.pyImportPrefix + dh + idlName + " import " + idPython
                .className(idlName) + "Helper"
            )
          }
          getRef(tm.args.head)
        }
          ...
          d.defType match {
            case DInterface =>
              refs.add(
                "from " + spec.pyImportPrefix + idPython.local(
                  d.name
                ) + " import " + idPython.className(d.name) + "Helper"
              )
            case DRecord =>
              refs.add(
                "from " + spec.pyImportPrefix + idPython.local(
                  d.name
                ) + " import " + idPython.className(d.name)
              )
        ...
        case "list" | "set" =>
          idPython.local(
            baseTy + (if (tm.args.isEmpty) ""
                      else "_" + toPythonType(tm.args.head))
          )
        case "map" =>
          idPython.local(
            baseTy + (if (tm.args.isEmpty) ""
                      else
                        "_" + toPythonType(tm.args.head)) + "_" + toPythonType(
              tm.args(1)
            )
          )
        case _ =>
  ...
  def getPacked(
    ...
    arg.base match {
      case MString =>
        "CPyString.fromPy" + opt_s + p(argName) + " as " + pyArgName
      case MBinary =>
        "CPyBinary.fromPy" + opt_s + p(argName) + " as " + pyArgName
      case _ => throw new NotImplementedError()
    }
  }
      ...
      case MOptional => {
        arg.ty.resolved.args.head.base match {
          case MPrimitive(_, _, _, _, _, _, _, _, _, _) | MDate =>
            val idlName =
              arg.ty.resolved.args.head.base.asInstanceOf[MOpaque].idlName
            "CPyBoxed" + idPython.className(idlName) + ".fromPyOpt" + p(
              argName
            ) + " as " + pyArgName
          case _ =>
            getPacked(
              arg.ty.resolved.args.head,
              isOpt = true,
              argName,
              pyArgName
            )
  ...
  def getClassHoldingCDataSet(tm: MExpr): String = tm.base match {
    case MList | MSet | MMap =>
      idPython.className(getExprIdlName(tm)) + "Helper"
    ...
    ty.base match {
      ...
      case _ =>
        fromRAII(
          ...
          ty,
          isOpt
        ) // nothing else needs to be released yet, the asserts for empty c_data_sets would fail otherwise
  ...
  def convertTo(name: String, ty: TypeRef): String =
    convertTo(name, ty.resolved, false)
  def convertTo(name: String, ty: MExpr, isOpt: Boolean): String = {
    ...
    ty.base match {
      ...
      case MList =>
        "CPyObject.toPy" + opt_s + p(
          idlName + "Helper" + ".c_data_set" + ", " + local
        )
      case MMap | MSet =>
        "CPyObjectProxy.toPyObj" + opt_s + p(
          idlName + "Helper" + ".c_data_set" + ", " + local
        )
      case d: MDef =>
        d.defType match {
          case DInterface =>
            idPython.className(d.name) + "Helper" + ".toPy" + p(local)
          case DRecord =>
            "CPyRecord.toPy" + opt_s + p(
              idPython.className(d.name) + ".c_data_set" + ", " + name
            ) // TODO different in that it does not .local, why would it
          case DEnum =>
            "CPyEnum.toPy" + opt_s + p(idPython.className(d.name) + ", " + name)
        }
      case MOptional => {
  ...
  def convertToRelease(name: String, ty: TypeRef): String =
    convertToRelease(name, ty.resolved)
  def convertToRelease(name: String, ty: MExpr): String = {
    val local = idPython.local(name)
    val idlName = idPython.className(getExprIdlName(ty))
    ty.base match {
      case MString | MBinary =>
        "CPy" + idlName + ".toPyWithoutTakingOwnership" + p(local)
      case MOptional =>
        ty.args.head.base match {
          case m @ (MPrimitive(_, _, _, _, _, _, _, _, _, _) | MDate) =>
            val idlName = m.asInstanceOf[MOpaque].idlName
            "CPyBoxed" + idPython.className(
              idlName
            ) + ".toPyOptWithoutTakingOwnership" + p(local)
  ...
  def convertFrom(name: String, ty: TypeRef): String =
    convertFrom(name, ty.resolved, isOpt = false)
  def convertFrom(name: String, ty: MExpr, isOpt: Boolean): String = {
    ...
    ty.base match {
      ...
      case MList =>
        "CPyObject.fromPy" + opt_s + p(
          idlName + "Helper" + ".c_data_set, " + local
        )
      case MMap | MSet =>
        "CPyObjectProxy.fromPy" + opt_s + p(
          idlName + "Helper" + ".c_data_set, " + idlName + "Proxy" + p(local)
        )
      case d: MDef =>
        d.defType match {
          case DInterface =>
            idPython.className(d.name) + "Helper" + ".fromPy(" + local + ")"
          case DRecord =>
            "CPyRecord.fromPy" + opt_s + p(
              idPython.className(d.name) + ".c_data_set, " + local
            )
