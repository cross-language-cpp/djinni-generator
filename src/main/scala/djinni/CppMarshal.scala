...
class CppMarshal(spec: Spec) extends Marshal(spec) {
  ...
  def typename(tm: MExpr, scopeSymbols: Seq[String]): String =
    toCppType(tm, None, scopeSymbols)
  ...
  def paramType(tm: MExpr, scopeSymbols: Seq[String]): String =
    toCppParamType(tm, None, scopeSymbols)
  ...
  override def fqParamType(tm: MExpr): String =
    toCppParamType(tm, Some(spec.cppNamespace))
  ...
  override def returnType(ret: Option[TypeRef]): String =
    ret.fold("void")(toCppType(_, None))
  override def fqReturnType(ret: Option[TypeRef]): String = {
    ret.fold("void")(toCppType(_, Some(spec.cppNamespace)))
  }

  def fieldType(tm: MExpr, scopeSymbols: Seq[String]): String =
    typename(tm, scopeSymbols)
  def fieldType(ty: TypeRef, scopeSymbols: Seq[String]): String =
    fieldType(ty.resolved, scopeSymbols)
  ...
  override def toCpp(tm: MExpr, expr: String): String =
    throw new AssertionError("cpp to cpp conversion")
        ...
        case r: Record =>
          if (d.name != exclude) {
            if (forwardDeclareOnly) {
              List(
                DeclRef(
                  s"struct ${typename(d.name, d.body)};",
                  Some(spec.cppNamespace)
                )
              )
            } else {
        ...
        case e: Enum =>
          if (d.name != exclude) {
            if (forwardDeclareOnly) {
              ...
              List(
                DeclRef(
                  s"enum class ${typename(d.name, d.body)}${underlyingType};",
                  Some(spec.cppNamespace)
                )
              )
            } else {
        ...
        case i: Interface =>
          val base = if (d.name != exclude) {
            List(
              ImportRef("<memory>"),
              DeclRef(
                s"class ${typename(d.name, d.body)};",
                Some(spec.cppNamespace)
              )
            )
          } else {
  ...
  private def toCppType(
    ...
    def withNamespace(name: String): String = {
      ...
      val ns = namespace match {
        ...
        case None =>
          if (scopeSymbols.contains(name)) Some(spec.cppNamespace) else None
      }
      withNs(ns, name)
    }
    def base(m: Meta): String = m match {
      ...
      case d: MDef =>
        d.defType match {
          ...
          case DInterface =>
            s"std::shared_ptr<${withNamespace(idCpp.ty(d.name))}>"
        }
      case e: MExtern =>
        e.defType match {
        ...
        case Some(nnType) => {
          ...
          tm.base match {
            case d: MDef =>
              d.defType match {
                case DInterface =>
                  s"${nnType}<${withNamespace(idCpp.ty(d.name))}>"
